...
export default function CallController() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-3xl mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-display font-bold text-foreground">Call Controller</h1>
        <p className="text-sm text-muted-foreground">Connect with your finance team</p>
      </div>
        ...
        <Card className="border-0 shadow-sm bg-primary text-primary-foreground">
          <CardContent className="p-6 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="font-semibold">Calling {calling}...</p>
      ...
      <Card className="border-0 shadow-sm">
        <CardHeader className="pb-2">
          <CardTitle className="text-base font-display flex items-center gap-2">
            <FileText className="h-4 w-4" /> AI Call Briefing
          </CardTitle>
        </CardHeader>
        <CardContent>
          <Button variant="outline" size="sm" className="mb-3" onClick={() => setShowBrief(!showBrief)}>
            {showBrief ? 'Hide Briefing' : 'Generate Briefing'}
          </Button>
          {showBrief && (
            <div className="bg-accent rounded-lg p-4 text-sm text-foreground space-y-2 animate-fade-in">
              <p className="font-medium">📋 Pre-Call Summary:</p>
              <p className="text-muted-foreground">{generateInsight('monthly', 'all')}</p>
              <p className="text-muted-foreground">Key discussion points: Q4 budget allocation, vendor contract renewals, capital expenditure approval for new infrastructure.</p>
            </div>
          )}
        </CardContent>
      </Card>
